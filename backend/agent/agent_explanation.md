...
└───┬───┬───┬──────┘
    ...
    └──── research ──► 正常解读路径（继续）
              ...
              ┌──────────────────┐
              │ ⑤ 并行解读        │  单牌同步，多牌线程池并行
              │   interpret      │  结合牌面意象直接回应
              └────────┬─────────┘
                       │
                       ▼
              ┌──────────────────┐
              │ ⑥ 自我反思        │  审核解读质量
              │   self_reflect   │  (画面感/针对性/牌义融合)
              └───┬───┬───┬──────┘
                  │   │   │
...
始终执行，不依赖路由。LLM 读取用户问题，用一句话提炼背后真实诉求（如"用户担忧职业发展瓶颈"），同时调用 `classify_intent` 将问题归类为五大意图之一。单牌场景的意图分类结果不影响牌阵规划，但仍注入到解读 prompt 中作为参考。
...
- **clarify**（追问）：问题过于模糊或宽泛（如仅说"我怎么办""帮帮我"）。行为取决于 `spread_type`：
  - `single_card` / `daily_card`：生成一句温和追问引导用户**下次抽牌前**提出更具体的问题，**直接结束**。
...
解读风格：结合牌面意象回应来访者的困惑。禁止使用「我看到」「画面中」「启示」等 AI 痕迹字眼。不超过 200 字。
...
仅 `three_card` 执行。将三张牌的意象串联为一段回应，直击来访者的困惑。`daily_card` 和 `single_card` 跳过此节点，`synthesis` 返回 `null`。不超过 180 字。

#### ⑧ 生成建议（`gen_advice`）

LLM 输出 1-2 条简短可行的建议。敏感内容场景下引导用户关注内心感受而非寻求专业结论。

#### ⑨ 生成摘要（`gen_summary`）

LLM 用一句话直接回应来访者的问题。不超过 35 字（三牌 45 字）。敏感内容场景下摘要前自动添加免责前缀 `"塔罗映照内心，但无法替代专业判断。"`。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `status` | `"success"` / `"awaiting_clarify"` | `success`：解读完成或婉拒；`awaiting_clarify`：三牌追问，等待用户补充 |
...
`user_supplement` 会被注入到解读（`interpret_cards`）、综合叙事（`synthesize`）、行动建议（`generate_advice_list`）三个阶段的 prompt 中，使解读更有针对性。
