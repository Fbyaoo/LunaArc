...
当前 YOLO 模型还可以识别其他手势类别，但这些类别不属于本项目业务交互协议。

对于非有效手势，视觉模块不得输出业务事件，应该直接忽略，最终表现为：
...
返回 `[]` 就代表当前帧没有可用的稳定业务手势。

如果有效手势名称发生变化，必须先同步后端协议。
...
其他类别会被视觉模块忽略，不输出 `gesture_event`。
...
```bash
python vision/test/test_vision_image.py vision/test/test_images/example.jpg
```

也可以运行项目内置的轻量协议测试：

```bash
python -m unittest discover -s vision/test -p "test_*.py"
```
